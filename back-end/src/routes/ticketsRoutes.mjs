...
class TicketRoutes {
    ...
    initRoutes() {
        ...
        /**
         * Route to close a ticket, called when the counter or customer
         * 
         * Return codes:
         * 500 - Internal Server Error
         * 409 - Conflict
         ...
         * 200 - OK
         */
        this.router.post("/:ticketId/cancel", async (req, res) => {
            try {
                const ticketId = Number(req.params.ticketId);
                if (!Number.isInteger(ticketId) || ticketId <= 0) {
                    return res.status(400).json({ error: "Invalid or missing ticketId" });
                ...
                const changes = await this.ticketDAO.cancelTicket(ticketId);
                if (changes === 0) {
                    // No update: ticket not in IN PROGRESS status or already closed or non-existent
                    return res.status(409).json({ error: "Ticket not in progress or already closed" });
                }
